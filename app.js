...
function renderTimer(){els.time.textContent=format(secondsLeft);document.title=`${format(secondsLeft)} · Foco`;els.ring.style.strokeDashoffset=861*(1-secondsLeft/totalSeconds);els.mode.textContent=mode==='focus'?'momento de enfocarse':'momento de respirar';els.session.textContent=mode==='focus'?`sesión ${state.session} de 4`:(state.session===4?'descanso largo':'descanso corto');els.note.innerHTML=`Tu ritmo recomendado: <strong>${recommendation()} min</strong>`}
...
function toggle(){running=!running;els.playIcon.textContent=running?'Ⅱ':'▶';els.playText.textContent=running?'pausar':'continuar';if(running){timerId=setInterval(tick,1000)}else clearInterval(timerId)}
function tick(){secondsLeft--;renderTimer();if(secondsLeft<=0)completeSession()}
function completeSession(){clearInterval(timerId);running=false;notify();if(mode==='focus'){const s=dayStats();s.minutes+=Math.round(totalSeconds/60);s.sessions++;state.completions++;mode='break'}else{mode='focus';state.session=state.session===4?1:state.session+1}resetClock();save();renderAll();toast(mode==='break'?'¡Sesión completada! Hora de respirar.':'Descanso listo. Volvamos con calma.')}
function resetClock(){clearInterval(timerId);running=false;const mins=mode==='focus'?recommendation():(state.session===4?state.settings.long:state.settings.short);totalSeconds=mins*60;secondsLeft=totalSeconds;els.playIcon.textContent='▶';els.playText.textContent='empezar'}
...
function toggleTask(id){const task=state.tasks.find(t=>t.id===id);task.done=!task.done;if(task.done){dayStats().tasks++;toast('Pequeña victoria registrada ✓')}else dayStats().tasks=Math.max(0,dayStats().tasks-1);save();renderAll()}
function deleteTask(id){state.tasks=state.tasks.filter(t=>t.id!==id);save();renderTasks()}
function notify(){if('Notification'in window&&Notification.permission==='granted')new Notification('Foco',{body:'Tu sesión terminó. Es hora de cambiar de ritmo.'});if($('soundButton').classList.contains('active')){const ctx=new AudioContext(),osc=ctx.createOscillator(),gain=ctx.createGain();osc.frequency.value=620;gain.gain.setValueAtTime(.15,ctx.currentTime);gain.gain.exponentialRampToValueAtTime(.001,ctx.currentTime+.7);osc.connect(gain).connect(ctx.destination);osc.start();osc.stop(ctx.currentTime+.7)}}
